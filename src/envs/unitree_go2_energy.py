...
class UnitreeGo2Env(PipelineEnv):
    ...
    def step(self, state: State, action: jax.Array) -> State:  # pytype: disable=signature-mismatch
        ...
        rewards = {
            ...
            'survival': jnp.float64(
                self._reward_survival(done)
    ...
    def _reward_tracking_velocity(
        ...
        # Tracking of forward linear velocity target
        base_velocity = math.rotate(xd.vel[0], math.quat_inv(x.rot[0]))
        return jnp.abs(commands - base_velocity[0])

    def _reward_survival(self, done: jax.Array) -> jax.Array:
        return self.velocity_target * (~done)
